...
//    tempFile.toFile().deleteOnExit();
   ...
   *                              the schema
   */
  public static void validate(Path xml, Path schema) throws IOException, InterruptedException, Exception {
    ...
    /**
     * if the WGEt process exited cleanly then move the temporary file either to
     * the destination (if configured) or to the (temporary) download directory,
     * preserving the source file name from the URL.
     */
    if (exitCode == 0) {
